...
describe("reference window model", () => {
  it("appends older pages in order and lets the fetched page refresh overlap metadata", () => {
    expect(
      appendOlderReferences(
        [item("a"), item("b", 1)],
        [item("b", 2), item("c")],
  ...
  it("prepends newer pages and keeps the fetched newer copy at overlap", () => {
    expect(
      prependNewerReferences(
        [item("c", 1), item("d")],
        [item("a"), item("b"), item("c", 2)],
      ),
    ).toEqual([item("a"), item("b"), item("c", 2), item("d")]);
  });
