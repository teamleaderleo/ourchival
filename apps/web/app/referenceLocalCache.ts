...
export async function loadCachedReferencePage(args: {
  ...
  try {
    const value = await requestResult<unknown>(
      database
        .transaction(pageStore, "readonly")
...
async function pruneReferenceCache(database: IDBDatabase) {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    request.onerror = () => reject(request.error ?? new Error("Reference cache cursor failed."));
    request.onsuccess = () => {
...
async function openReferenceCache() {
  ...
  return await new Promise<IDBDatabase>((resolve, reject) => {
    ...
    request.onerror = () => reject(request.error ?? new Error("Could not open reference cache."));
