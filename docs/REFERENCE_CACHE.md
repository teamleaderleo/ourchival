...
Endless scrolling should accumulate cursor pages without duplicating references at overlap boundaries.

Navigation direction owns sequence order. The newly fetched page owns the hydrated metadata for any overlapping reference ID. The in-memory window can then be bounded around the selected/visible anchor independently from server pagination.
...
6. use the page-window model to accumulate older/newer cursor pages;
7. wire viewport/selection changes into the prefetch planner;
8. then start the virtualized endless-gallery work.
